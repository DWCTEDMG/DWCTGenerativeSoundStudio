...
| P0-02 | In progress / blocked evidence | Repository admin approval | `.github/**`; `CONTRIBUTING.md`; `RELEASE.md`; branch-policy docs | Policy-doc checks; GitHub branch/settings inspection | Remote default is `codex/Unified`; required checks and protected `main` require an approved settings change and push |
| P0-03 | In progress | License policy | `.gitignore`; `.env.example`; `LICENSE`; `SECURITY.md`; `CHANGELOG.md` | Ignore proof, secret scan, doc/link checks | Repository currently has no declared license; preserve the existing all-rights-reserved state unless the owner approves another license |
