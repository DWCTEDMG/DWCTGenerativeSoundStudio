...
| Live worktree test edits | `test_tensorrt_video.py` and `test_workspace_reactive_integration.py` contain uncommitted fixture/monkeypatch edits; the focused run currently reports 61 passed and 2 failed | Preserve for review; do not publish or mark the related workflow gate complete until the failing expectations are resolved and rerun. |
