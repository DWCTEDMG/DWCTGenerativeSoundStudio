...
$AcceleratorProfile = ([string]$BackendManifest.acceleratorProfile).Trim().ToLowerInvariant()
if ($AcceleratorProfile -notmatch "^[a-z0-9][a-z0-9_-]*$") {
  throw "Packaged backend manifest declares a filename-unsafe accelerator profile."
}
if ($AcceleratorProfile -notin @("cpu", "directml", "cuda")) {
  throw "Inno packaging requires acceleratorProfile cpu, directml, or cuda; found $AcceleratorProfile."
}
...
$PreviousSetupPatterns = @(
  "EDMG-Studio-Setup-*.exe",
  "EDMG-Studio-*-windows-x64-*-Setup.exe"
)
foreach ($setupPattern in $PreviousSetupPatterns) {
  foreach ($previousSetup in Get-ChildItem -LiteralPath $OutDirAbs -File -Filter $setupPattern) {
