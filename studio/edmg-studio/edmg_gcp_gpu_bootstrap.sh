...
export HUGGINGFACE_ASSETS_CACHE=${HF_ASSETS_CACHE_DIR}
...
ensure_directories() {
  ...
  "${SUDO[@]}" mkdir -p "${REPO_PARENT}" "${DATA_DIR}" "${MODELS_DIR}" "${CACHE_DIR}" "${LOG_DIR}" "${EXTERNAL_DIR}" "${CONFIG_DIR}" "${HF_HUB_CACHE_DIR}" "${HF_XET_CACHE_DIR}" "${HF_ASSETS_CACHE_DIR}" "${TRANSFORMERS_CACHE_DIR}" "${TORCH_HOME_DIR}" "${OLLAMA_MODELS_DIR}" "${BIN_DIR}"
