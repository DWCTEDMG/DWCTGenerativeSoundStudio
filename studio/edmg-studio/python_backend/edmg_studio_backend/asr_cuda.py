...
def preload_cuda12_cublas() -> tuple[str, ...]:
    """Reuse installed, complete CUDA 12 pairs; retain their process handles."""
    ...
    with _lock:
        ...
        for directory in _candidate_directories():
            ...
            if not all(path.is_file() for path in paths):
                continue
            # Load dependencies first, by absolute path. Keep any successfully
            # loaded handle even if the second DLL fails; do not mix candidates.
            try:
                for path in paths:
                    _handles.append(ctypes.WinDLL(str(path.resolve())))
            except OSError as exc:
                raise RuntimeError(f"CUDA transcription could not load the cuBLAS 12 pair in {directory}: {exc}") from exc
