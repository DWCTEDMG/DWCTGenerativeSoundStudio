...
public sealed class StudioApiClient : IDisposable
{
    ...
    public Task<JsonElement> GetHunyuanRuntimeConfigAsync(CancellationToken cancellationToken = default) =>
        SendJsonElementAsync(HttpMethod.Get, "/v1/runtimes/hunyuan-video15/config", null, true, cancellationToken);

    public Task<JsonElement> SaveHunyuanRuntimeConfigAsync(JsonElement request, CancellationToken cancellationToken = default) =>
        PostJsonElementAsync("/v1/runtimes/hunyuan-video15/config", request, cancellationToken);
